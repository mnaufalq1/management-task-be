...
export const getProjectMembers = async (req, res) => {
    try {
        ...
        const members = await projectMemberService.findAllProjectMembers(project_id, user_id);
...
export const createProjectMember = async (req, res) => {
    try {
        ...
        res
            .status(201)
            .json(successResponse(newMember, "Project member berhasil ditambahkan"));
    }
    catch (err) {
...
export const updateProjectMember = async (req, res) => {
    try {
        ...
        if (project_id === undefined && user_id === undefined) {
            res
                .status(400)
                .json(errorResponse("Setidaknya kirimkan project_id atau user_id untuk di-update"));
            return;
        }
        const updatedMember = await projectMemberService.updateProjectMemberById(id, req.body);
        if (!updatedMember) {
            res.status(404).json(errorResponse("Project member tidak ditemukan"));
            return;
        }
        res.json(successResponse(updatedMember, "Project member berhasil diperbarui"));
    }
    catch (err) {
...
export const deleteProjectMember = async (req, res) => {
    try {
        ...
        res.json(successResponse(deletedMember, "Project member berhasil dihapus"));
    }
    catch (err) {
